...
+++ GNUmakefile.inc.in	5 Jun 2006 18:09:17 -0000
@@ -12,13 +12,36 @@
 # Variables
 ########################################################################
...
+++ matlab.hpp	5 Jun 2006 18:09:17 -0000
@@ -0,0 +1,277 @@
...
--- /dev/null	1 Jan 1970 00:00:00 -0000
+++ matlab_bin_formatter.hpp	5 Jun 2006 18:09:18 -0000
@@ -0,0 +1,372 @@
...
+        o.write(reinterpret_cast<char*>(&temp_data_element),
+                  sizeof(temp_data_element));
+        if(i==0) matlab::write<T,r_v>(o,subview::real(mbf.v));
+        else     matlab::write<T,i_v>(o,subview::imag(mbf.v));
+      }
+    }
+  }
+
...
+    {
+      if(i==0)matlab::read<int16_t,T,r_v>(is,subview::real(mbf.v));
+      else    matlab::read<int16_t,T,i_v>(is,subview::imag(mbf.v));
+    }
+    else if(temp_data_element.type == matlab::miUINT16) 
+    {
+      if(i==0)matlab::read<uint16_t,T,r_v>(is,subview::real(mbf.v));
+      else    matlab::read<uint16_t,T,i_v>(is,subview::imag(mbf.v));
+    }
+    else if(temp_data_element.type == matlab::miINT32) 
+    {
+      if(i==0)matlab::read<int32_t,T,r_v>(is,subview::real(mbf.v));
+      else    matlab::read<int32_t,T,i_v>(is,subview::imag(mbf.v));
+    }
+    else if(temp_data_element.type == matlab::miUINT32) 
+    {
+      if(i==0)matlab::read<uint32_t,T,r_v>(is,subview::real(mbf.v));
+      else    matlab::read<uint32_t,T,i_v>(is,subview::imag(mbf.v));
+    }
+    else if(temp_data_element.type == matlab::miSINGLE) 
+    {
+      if(i==0)matlab::read<float,T,r_v>(is,subview::real(mbf.v));
+      else    matlab::read<float,T,i_v>(is,subview::imag(mbf.v));
+    }
+    else
+    {
+      if(i==0)matlab::read<double,T,r_v>(is,subview::real(mbf.v));
+      else    matlab::read<double,T,i_v>(is,subview::imag(mbf.v));
+    }
+
+  }
+
...
--- /dev/null	1 Jan 1970 00:00:00 -0000
+++ matlab_text_formatter.hpp	5 Jun 2006 18:09:18 -0000
@@ -0,0 +1,91 @@
+/* Copyright (c) 2005, 2006 by CodeSourcery.  All rights reserved. */
+
...
+++ output.hpp	5 Jun 2006 18:09:18 -0000
@@ -17,9 +17,6 @@
