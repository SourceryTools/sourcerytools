...
+  <entry><code>dot, dotcvj, trans, herm, kron, outer, gemp, gems, cumsum and modulate</code></entry>
 </row>
+<row>
+  <entry>fns</entry>
+  <entry><code>1-D, 2-D and 3-D Loop Fusion, Copy, Transpose, Dense Block, SIMD and vendor libraries</code></entry>
+</row>
 </tbody>
 </tgroup>
 </table>
...
+      the value <literal>15</literal>.  See <xref linkend="mask-values"/>
...
+<screen>--vsipl++-profile-mode=accum --vsipl++-profile-output=profile.txt
+</screen>
     </para>
...
+      to the frequency domain.  In particular, an FFT is applied to a data 
+      cube of 64 pulses, each containing 256 range cells:
+<screen>Fftm Fwd row_type C-C by_ref 64x256 : 1188144 : 1 : 1146880 : 3466.65
+</screen>
...
+<screen>Expr_Loop_Vmmul 2D vmmul(C,C) 64x256 : 1539531 : 1 : 98304 : 229.321
+</screen> 
...
+<screen>Expr_SIMD_VV-simd::vmul 1D *(C,C) 256 : 316674 : 64 : 1536 : 1114.86
+</screen> 
+      The tag used for this expression is "*(C,C)".  The profiling tag for 
+      many operations is shown using a prefix notation; the operation 
+      performed is followed by the types of the arguments.  The "simd" tag 
+      indicates that VSIPL++ used the Single Instruction Multiple Data (SIMD) 
+      facilities on the Xeon architecture for maximum performance.
+    </para>
+    <para>
+      The tick count for the vector-matrix multiplication (vmmul) includes 
+      the time spent in the multiple row-wise scalar-vector multiplications. 
+      Therefore the total number of time used by the program is *not* the 
+      sum of the tick counts given for each line.
+    </para>
+    </section>
+    <section><title>Convert back to time domain</title>
+    <para>
+      The last step of the algorithm is to convert back to the time domain 
+      by using an inverse FFT.  An inverse FFT is computationally 
+      equivalent to a forward FFT, except that an additional multiplication 
...
+      the options
+<screen>--vsipl++-profile-mode=trace --vsipl++-profile-output=profile.txt
+</screen>
     </para>
...
+    <para>
+      In the break shown by the ellipses, the program is in the middle of
...
+      To create a <code>Scope_event</code>, call the constructor, passing 
+      it a std::string that will become the event tag and, optionally, an 
...
+<screen>Fast Convolution : 4256109 : 1 : 2424832 : 2046.11
+</screen>
+    </para>
+    <para>
...
+Fft 1D Fwd C-C by_ref     256x1 : 142119 : 1 : 10240 : 258.767
+Fftm 2D Fwd C-C by_ref      64x256 : 1188144 : 1 : 1146880 : 3466.65
+Fftm 2D Inv C-C by_ref      64x256 : 1559304 : 1 : 1146880 : 2641.48
...
+4 : Fftm Fwd row_type C-C by_ref 64x256 : 2996724027068541 : 0 : 1146880
+5 : Fftm Fwd row_type C-C by_ref 64x256 : 2996724028229361 : 4 : 0
...
+136 : Fftm Inv row_type C-C by_ref 64x256 : 2996724029724846 : 0 : 1146880
...
+141 : Fftm Inv row_type C-C by_ref 64x256 : 2996724031264422 : 136 : 0
...
+Fft Fwd C-C by_ref 256 : 81261 : 1 : 10240 : 452.562
+Fftm Fwd row_type C-C by_ref 64x256 : 1152891 : 1 : 1146880 : 3572.65
+Fftm Inv row_type C-C by_ref 64x256 : 1535049 : 1 : 1146880 : 2683.22
\ No newline at end of file
...
+    to interprete the resulting profiler data.
+  </para>
...
+      objects defined in user programs falls into the 'user' category
+      of events.
+    </para>
+    <para>
...
+      (<xref linkend="event-tags"/> describes the event tags used 
...
+
+
+  <section><title>Profiler Output</title>
...
+<inlineequation>
+  <mathphrase>(num_calls * op_count * 10<superscript>-6</superscript>) / (total_ticks / clocks_per_sec)</mathphrase>
+</inlineequation>
+    	      </para>
+	    </listitem>
+	  </varlistentry>
...
+      containing information that varies depending on the event, but 
+      generally they follow this format:
+    </para>
+<screen>OPERATION [DIM] DATATYPE SIZE</screen>
+    <para> 
+      OPERATION gives the object or function name.
+    </para>
+    <para> 
...
+    </para>
+    <para>
+      Tags used for signal processing and matrix-vector operations
+      are as follows:
+    </para>
+<simplelist>
+<member><code>Convolution [1D|2D] T <replaceable>SIZE</replaceable></code></member>
+<member><code>Correlation [1D|2D] <replaceable>T</replaceable> <replaceable>SIZE</replaceable></code></member>
+<member><code>Fft 1D [Inv|Fwd] <replaceable>I</replaceable>-<replaceable>O</replaceable> [by_ref|by_val] <replaceable>SIZE</replaceable></code></member>
+<member><code>Fftm 2D [Inv|Fwd] <replaceable>I</replaceable>-<replaceable>O</replaceable> [by_ref|by_val] <replaceable>SIZE</replaceable></code></member>
+<member><code>Fir <replaceable>T</replaceable> <replaceable>SIZE</replaceable></code></member>
+<member><code>Iir <replaceable>T</replaceable> <replaceable>SIZE</replaceable></code></member>
+<member><code>dot <replaceable>T</replaceable> <replaceable>SIZE</replaceable></code></member>
+<member><code>cvjdot <replaceable>T</replaceable> <replaceable>SIZE</replaceable></code></member>
+<member><code>trans <replaceable>T</replaceable> <replaceable>SIZE</replaceable></code></member>
+<member><code>herm <replaceable>T</replaceable> <replaceable>SIZE</replaceable></code></member>
+<member><code>kron <replaceable>T</replaceable> <replaceable>SIZE A</replaceable> <replaceable>SIZE B</replaceable></code></member>
+<member><code>outer <replaceable>T</replaceable> <replaceable>SIZE</replaceable></code></member>
+<member><code>gemp <replaceable>T</replaceable> <replaceable>SIZE</replaceable></code></member>
+<member><code>gems <replaceable>T</replaceable> <replaceable>SIZE</replaceable></code></member>
+<member><code>cumsum <replaceable>T</replaceable> <replaceable>SIZE</replaceable></code></member>
+<member><code>modulate <replaceable>T</replaceable> <replaceable>SIZE</replaceable></code></member>
+</simplelist>
+    <para>
