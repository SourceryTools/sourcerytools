...
+++ layout.hpp	19 Jun 2006 11:33:37 -0000
@@ -1089,6 +1089,12 @@
...
+++ matlab.hpp	19 Jun 2006 11:34:49 -0000
@@ -0,0 +1,291 @@
...
+  };
+
+  // endian stuff. These functions swap bytes if we need to
+  std::string get_machine_endian()
+  {
+    uint32_t a = 0x12345678;
+    uint8_t  *p = reinterpret_cast<uint8_t*>(&a);
+
+    // look at byte 0
+    if(*p == 0x78) return "IM"; // little endian
+    else           return "MI";
+
+  }
+
...
+	    typename ViewT>
+  void read(std::istream& is,ViewT v,std::string endian)
+  {
...
--- /dev/null	1 Jan 1970 00:00:00 -0000
+++ matlab_bin_formatter.hpp	19 Jun 2006 11:34:49 -0000
@@ -0,0 +1,374 @@
...
+  for (int i=0;i <= vsip::impl::Is_complex<T>::value;i++)
+  {
+    std::string endian = mbf.header.endian;
+
+    // read data header
...
--- /dev/null	1 Jan 1970 00:00:00 -0000
+++ matlab_text_formatter.hpp	19 Jun 2006 11:34:49 -0000
@@ -0,0 +1,91 @@
+/* Copyright (c) 2005, 2006 by CodeSourcery.  All rights reserved. */
+
...
+++ output.hpp	19 Jun 2006 11:34:49 -0000
@@ -114,6 +114,6 @@
   return out;
 }
