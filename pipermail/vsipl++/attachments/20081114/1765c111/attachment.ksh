Index: ChangeLog
===================================================================
--- ChangeLog	(revision 227758)
+++ ChangeLog	(working copy)
@@ -1,3 +1,12 @@
+2008-11-14  Jules Bergmann  <jules@codesourcery.com>
+
...
===================================================================
--- apps/ssar/kernel1.hpp	(revision 227758)
...
+  fftshift_0(fs_ref_, fs_ref_preshift_);
+  fd_fftshift_1(fs_ref_preshift_, fs_ref_preshift_);
...
@@ -370,6 +421,10 @@
 ...
 {
+  using vsip_csl::matlab::fftshift_1;
+  using vsip_csl::matlab::fd_fftshift_0;
+  using vsip_csl::matlab::fd_fftshift_1;
+
...
+#if !DIGITAL_SPOTLIGHT_BY_ROW
+  fftshift_1(s_compr_filt_, s_compr_filt_shift_);
+  fd_fftshift_0(s_compr_filt_shift_, s_compr_filt_shift_);
+#endif
...
+#if !DIGITAL_SPOTLIGHT_BY_ROW
+  fftshift_1(s_decompr_filt_, s_decompr_filt_shift_);
+  fd_fftshift_1(s_decompr_filt_shift_, s_decompr_filt_shift_);
+#endif
...
+  {
+    Scope<user> scope("decompr-halfast",  slow_time_decompression_ops_);
...
-  SAVE_VIEW("p77_fsm.view", fsm_, this->swap_bytes_);
+  {
+    Scope<user> scope("st-halfast",  slow_time_decompression_ops_);
+    fsm_ = this->fs_ref_preshift_ * st_fftm_(fs_padded_); // row
+  }
+
+  SAVE_VIEW("p77_fsm_halfast.view", fsm_, this->swap_bytes_);
 }
...
===================================================================
--- apps/ssar/ssar.cpp	(revision 227758)
...
+  huge_map.impl_set_pool(
+    new vsip::impl::Huge_page_pool("/huge/benchmark.bin", 20));
+#endif
+
...
===================================================================
--- src/vsip_csl/matlab_utils.hpp	(revision 227758)
...
+	  typename Matrix2T>
+void
+fd_fftshift_0(Matrix1T in, Matrix2T out)
+{
+  typedef typename Matrix1T::value_type T;
+  length_type rows = in.size(0);
+  length_type cols = in.size(1);
...
+	  typename Matrix2T>
+void
+fd_fftshift_1(Matrix1T in, Matrix2T out)
+{
+  typedef typename Matrix1T::value_type T;
+  length_type rows = in.size(0);
+  length_type cols = in.size(1);
