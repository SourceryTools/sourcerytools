Index: ../vpp/doc/manual/datatypes.xml
===================================================================
--- ../vpp/doc/manual/datatypes.xml	(revision 222103)
+++ ../vpp/doc/manual/datatypes.xml	(working copy)
@@ -756,6 +756,67 @@
    </variablelist>
   </section>
...
+   and preferred for larger data sets.</para>
+
+   <variablelist>
+    <varlistentry>
+     <term>by_value</term>
...
Index: ../vpp/doc/manual/functions.xml
===================================================================
--- ../vpp/doc/manual/functions.xml	(revision 222103)
+++ ../vpp/doc/manual/functions.xml	(working copy)
@@ -121,6 +121,11 @@
  </section>
...
+
+ <section>
   <title>Matrix-Vector Linear Algebra Functions</title>
   <xi:include href="cumsum.xml" />
   <xi:include href="cvjdot.xml" />
Index: ../vpp/doc/manual/qr.xml
===================================================================
--- ../vpp/doc/manual/qr.xml	(revision 0)
+++ ../vpp/doc/manual/qr.xml	(revision 0)
@@ -0,0 +1,844 @@
...
+
+    <listitem>
+     <para>The return mechanism type <type>by_value</type> indicates
+     <type>qrd</type> returns a result matrix by value, 
+     <type>by_reference</type> indicates results are returned by reference
+     in a matrix provided by the caller.
+     See <xref 
...
+   </indexterm>
+
+   <synopsis>     qrd(length_type rows, length_type columns, storage_type st);</synopsis>
+
+   <formalpara>
...
+    <title>Description:</title>
+
+    <para>Report the various attributes of this <type>qrd</type> object.  The
...
+     into matrices <varname>Q</varname> and <varname>R</varname>.</para>
+   </formalpara>
+   
+   <formalpara>
+     <title>Requirements:</title>
+     
+     <para>The matrix <parameter>A</parameter> may be overwritten.  It must 
+     be the same size as specified in the constructor.  </para>
+   </formalpara>
+   
+   <formalpara>
+     <title>Result:</title>
...
+
+   <para>The following functions are available only if the <type>qrd</type>
+   template class is parameterized with <code>ReturnMechanism=by_value</code>
+   </para>
+
+   <indexterm>
+    <primary>qrd</primary>
...
+     the table below) depends on the values of <type>tr</type>, 
+     <type>ps</type>, and <function>qstorage()</function> 
+     and whether <type>T</type> is not or is a specialization 
+     of complex.</para>
+   </formalpara>
+   <para>For <code>qstorage() == qrd_saveq1,</code></para>
+
...
+     </informaltable>
+
+   <para>where <parameter>s</parameter> is an arbitrary positive value.</para>
+   <para>For <code>qstorage() == qrd_saveq</code>,</para>
+
+     <informaltable pgwide="1">
+       <tgroup cols="3">
+	 <thead>
...
+     <type>qrd_saveq1</type> or <type>qrd_saveq</type>.  Otherwise, 
+     the behavior is undeﬁned.  The number of rows and columns (shown 
+     in the table below) of <parameter>m</parameter> depend on the 
+     values of <type>tr</type>, <type>ps</type>, and 
+     <function>qstorage()</function>.</para>
+   </formalpara>
+   <para>For <code>qstorage() == qrd_saveq1</code>,</para>
+
+     <informaltable pgwide="1">
...
+     </informaltable>
+
+   <para>where <parameter>s</parameter> is the same variable as above.</para>
+   <para>For <code>qstorage() == qrd_saveq</code>,</para>
+
+     <informaltable pgwide="1">
+       <tgroup cols="3">
...
+     Note that the returned matrix’s block type may be a different type from 
...
+     behavior is undeﬁned.  The number of rows and columns (shown in 
+     the table below) of <parameter>m</parameter> depend on the values of
+     <type>tr</type>, <type>ps</type>, and <function>qstorage()</function>.</para>
+   </formalpara>
+   <para>For <code>qstorage() == qrd_saveq1,</code></para>
+
+     <informaltable pgwide="1">
...
+   (shown in the table below) depend on the values of <type>tr</type>, 
+   <type>ps</type>, and <function>qstorage()</function>.</para>
+   <para>For <code>qstorage() == qrd_saveq1,</code></para>
+
+     <informaltable pgwide="1">
+       <tgroup cols="3">
