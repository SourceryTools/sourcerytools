...
+++ GNUmakefile.inc.in	24 May 2006 16:53:14 -0000
@@ -12,13 +12,36 @@
 # Variables
 ########################################################################
...
+++ matlab.hpp	24 May 2006 16:53:14 -0000
@@ -0,0 +1,98 @@
...
--- /dev/null	1 Jan 1970 00:00:00 -0000
+++ matlab_bin_formatter.hpp	24 May 2006 16:53:14 -0000
@@ -0,0 +1,359 @@
...
+  typedef typename vsip::impl::Scalar_of<T>::type scalar_type;
+  matlab::data_element temp_data_el;
+  int    num_points = mbf.v.size(0)*mbf.v.size(1);
+  int    sz;
...
+
+  // matrix data type
+  m_mtrx.header.type = matlab::miMATRIX;
+  m_mtrx.header.size = 1; // TEMP
+
+  // array flags
+  m_mtrx.array_flags_header.type = matlab::miUINT32;
+  m_mtrx.array_flags_header.size = 8;
+  if(vsip::impl::Is_complex<T>::value) 
...
+  
+  // dimension sizes
+  m_mtrx.dim_header.type = matlab::miINT32;
+  m_mtrx.dim_header.size = 8;
+  m_mtrx.dim1 = mbf.v.size(0);
+  m_mtrx.dim2 = mbf.v.size(1);
+
+  // array name
+  m_mtrx.array_name_header.type = matlab::miINT8;
+  m_mtrx.array_name_header.size = mbf.view_name.length();
+
+
+  // calculate size
+  sz = sizeof(m_mtrx)-8;
...
+    temp_data_el.type = matlab::miSINGLE;
+  else
+    temp_data_el.size = matlab::miDOUBLE;
+
+  temp_data_el.size = sizeof(scalar_type)*num_points;
+  o.write(reinterpret_cast<char*>(&temp_data_el),sizeof(temp_data_el));
+  
...
+
+    // Matlab wants data in col major format
+    for(vsip::length_type i=0;i<mbf.v.size(1);i++) {
+      for(vsip::length_type j=0;j<mbf.v.size(0);j++) {
+        real_data = vsip::impl::fn::impl_real(mbf.v.get(j,i));
+        o.write(reinterpret_cast<char*>(&real_data),sizeof(real_data));
+      }
+    }
+  }
...
+    for(vsip::length_type i=0;i<mbf.v.size(1);i++) {
+      for(vsip::length_type j=0;j<mbf.v.size(0);j++) {
+        imag_data = vsip::impl::fn::impl_imag(mbf.v.get(j,i));
+        o.write(reinterpret_cast<char*>(&imag_data),sizeof(imag_data));
+      }
+    }
+  }
...
+{
+  return o << 
+    Matlab_bin_formatter<vsip::const_Tensor<T,Block0> >(mbf.v,mbf.view_name);
+}
+
+template <typename T,
+          typename Block0>
...
+{
+  return o << 
+    Matlab_bin_formatter<vsip::const_Matrix<T,Block0> >(mbf.v,mbf.view_name);
+}
+
+template <typename T,
+          typename Block0>
...
+{
+  return o << 
+    Matlab_bin_formatter<vsip::const_Vector<T,Block0> >(mbf.v,mbf.view_name);
+}
+
+
+} // namespace vsip_csl
...
--- /dev/null	1 Jan 1970 00:00:00 -0000
+++ matlab_text_formatter.hpp	24 May 2006 16:53:14 -0000
@@ -0,0 +1,91 @@
+/* Copyright (c) 2005, 2006 by CodeSourcery.  All rights reserved. */
+
...
+++ output.hpp	24 May 2006 16:53:14 -0000
@@ -17,9 +17,6 @@
