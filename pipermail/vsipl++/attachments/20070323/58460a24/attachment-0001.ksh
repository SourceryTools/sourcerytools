...
+static float kernel[VSIP_IMPL_MAX_FCONV_SIZE*2] __attribute__ ((aligned (128)));
+
+// The twiddle factors occupy only 1/4 the space as the inputs, 
+// outputs and convolution kernels.
+static volatile float twiddle_factors[VSIP_IMPL_MAX_FCONV_SIZE*2/4] 
...
+static float kernel_re[VSIP_IMPL_MAX_FCONV_SPLIT_SIZE] __attribute__ ((aligned (128)));
+static float kernel_im[VSIP_IMPL_MAX_FCONV_SPLIT_SIZE] __attribute__ ((aligned (128)));
...
@@ -170,7 +172,7 @@
   ...
   unsigned int log2n = log2i(n);
-  assert(n <= MAX_FCONV_SIZE);
+  assert(n <= VSIP_IMPL_MAX_FCONV_SPLIT_SIZE);
