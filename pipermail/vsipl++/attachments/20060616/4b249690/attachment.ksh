...
+++ GNUmakefile.inc.in	16 Jun 2006 09:38:07 -0000
@@ -12,13 +12,36 @@
 # Variables
 ########################################################################
...
+++ matlab.hpp	16 Jun 2006 09:38:07 -0000
@@ -0,0 +1,248 @@
...
--- /dev/null	1 Jan 1970 00:00:00 -0000
+++ matlab_bin_formatter.hpp	16 Jun 2006 09:38:07 -0000
@@ -0,0 +1,372 @@
...
+    {
+      if(i==0)matlab::read<int8_t,sT,r_v>(is,subview::real(mbf.view));
+      else    matlab::read<int8_t,sT,i_v>(is,subview::imag(mbf.view));
+    }
+    else if(temp_data_element.type == matlab::miUINT8) 
+    {
+      if(i==0)matlab::read<uint8_t,sT,r_v>(is,subview::real(mbf.view));
+      else    matlab::read<uint8_t,sT,i_v>(is,subview::imag(mbf.view));
+    }
+    else if(temp_data_element.type == matlab::miINT16) 
+    {
+      if(i==0)matlab::read<int16_t,sT,r_v>(is,subview::real(mbf.view));
+      else    matlab::read<int16_t,sT,i_v>(is,subview::imag(mbf.view));
+    }
+    else if(temp_data_element.type == matlab::miUINT16) 
+    {
+      if(i==0)matlab::read<uint16_t,sT,r_v>(is,subview::real(mbf.view));
+      else    matlab::read<uint16_t,sT,i_v>(is,subview::imag(mbf.view));
+    }
+    else if(temp_data_element.type == matlab::miINT32) 
+    {
+      if(i==0)matlab::read<int32_t,sT,r_v>(is,subview::real(mbf.view));
+      else    matlab::read<int32_t,sT,i_v>(is,subview::imag(mbf.view));
+    }
+    else if(temp_data_element.type == matlab::miUINT32) 
+    {
+      if(i==0)matlab::read<uint32_t,sT,r_v>(is,subview::real(mbf.view));
+      else    matlab::read<uint32_t,sT,i_v>(is,subview::imag(mbf.view));
+    }
+    else if(temp_data_element.type == matlab::miSINGLE) 
+    {
+      if(i==0)matlab::read<float,sT,r_v>(is,subview::real(mbf.view));
+      else    matlab::read<float,sT,i_v>(is,subview::imag(mbf.view));
+    }
+    else
+    {
+      if(i==0)matlab::read<double,sT,r_v>(is,subview::real(mbf.view));
+      else    matlab::read<double,sT,i_v>(is,subview::imag(mbf.view));
+    }
+
+  }
+
...
--- /dev/null	1 Jan 1970 00:00:00 -0000
+++ matlab_text_formatter.hpp	16 Jun 2006 09:38:07 -0000
@@ -0,0 +1,91 @@
+/* Copyright (c) 2005, 2006 by CodeSourcery.  All rights reserved. */
+
...
+++ output.hpp	16 Jun 2006 09:38:07 -0000
@@ -114,6 +114,6 @@
   return out;
 }
