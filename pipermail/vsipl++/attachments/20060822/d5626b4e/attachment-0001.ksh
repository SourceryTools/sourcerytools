...
+1) Configuration Options
+-------------------------------------------------------------------------
+Enable a suitable high-resolution timer using:
+
+  --enable-timer=x86_64_tsc
+
+Enable compilation with profiling code included:
+
+  --enable-profiler=all
+
+Alternatively, you may specify a subset of VSIPL++ areas to profile
+using a combination of:
+
+  --enable-profiler={signal|matvec|fns|user}
+
+
+
+2) Command Line Options
...
+
+The second option defaults to /dev/stdout, so it may be omitted.
+
+
+
+3) Profiling Functions
...
+
+  impl::profile::Profile profile("/dev/stdout", impl::profile::accum)
+
+The 'Time_in_scope' object is used to insert a profiler event
...
+
+  impl::profile::Time_in_scope event("User Event", op_count);
+
