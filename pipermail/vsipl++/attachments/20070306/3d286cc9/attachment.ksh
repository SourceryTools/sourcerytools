...
===================================================================
--- benchmarks/alloc_block.hpp	(revision 0)
+++ benchmarks/alloc_block.hpp	(revision 0)
@@ -0,0 +1,271 @@
...
+  typedef vsip::Dense<Dim, T, order_type, MapT> block_type;
+
+  static block_type*
+  alloc(
...
+	  typename             T,
+	  typename             MapT>
+struct Alloc_block<Dim, std::complex<T>, vsip::impl::Cmplx_split_fmt, MapT>
+{
+  typedef typename vsip::impl::Row_major<Dim>::type order_type;
...
+    else
+    {
+      blk = new block_type(dom, 
+			vsip::impl::User_storage<T>(vsip::array_format,
+				(T*)(addr + offset)),
+			map);
+      blk->admit(false);
+      assert(!blk->is_alloc());
...
+    {
+      blk = new block_type(dom,
+			vsip::impl::User_storage<std::complex<T> >(
+				vsip::split_format, 0, 0),
+			map);
+
+      blk->rebind( (T*)(addr + offset) + 0,
+		   (T*)(addr + offset) + dom.size());
...
+	  typename       ComplexFmt,
+	  typename       MapT>
+typename Alloc_block<Dim, T, ComplexFmt, MapT>::block_type*
+alloc_block(
+  vsip::Domain<Dim> const& dom,
+  char*                    addr,
+  unsigned long            offset,
+  MapT const&              map = MapT())
+{
+  return Alloc_block<Dim, T, ComplexFmt, MapT>::alloc(dom, addr, offset, map);
+}
+
+
+
...
+	  typename       T,
+	  typename       ComplexFmt>
+typename Alloc_block<Dim, T, ComplexFmt, vsip::Local_map>::block_type*
+alloc_block(
+  vsip::Domain<Dim> const& dom,
+  char*                    addr,
+  unsigned long            offset)
+{
+  return Alloc_block<Dim, T, ComplexFmt, vsip::Local_map>
